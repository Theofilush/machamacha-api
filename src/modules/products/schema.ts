...
export const ProductSchema = z.object({
  id: z
    .string()
    .openapi({ example: "prod-001", description: "Unique product ID" }),
  name: z.string().min(1).openapi({ example: "Premium Matcha Latte" }),
  slug: z.string().openapi({ example: "premium-matcha-latte" }),
  price: z
    .number()
    .nonnegative()
    .openapi({ example: 75000, description: "Price in IDR" }),
  imageUrl: z.string().url().openapi({
    example: "https://cdn.machamacha.com/products/matcha-latte.jpg",
  }),
  description: z
    .string()
    .min(10)
    .openapi({ example: "A smooth and vibrant Japanese matcha latte blend." }),
